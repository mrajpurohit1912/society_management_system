...
def upgrade() -> None:
    ...
    # 1. Create complaint_tickets table if not exists
    if 'complaint_tickets' not in existing_tables:
        op.create_table(
            'complaint_tickets',
        ...
        existing_indexes = {ix['name'] for ix in inspector.get_indexes('complaint_tickets')}
        if 'ix_complaint_tickets_ticket_number' not in existing_indexes:
            try:
                op.create_index(op.f('ix_complaint_tickets_ticket_number'), 'complaint_tickets', ['ticket_number'], unique=True)
            except Exception:
                pass

    # 2. Create complaint_comments table if not exists
    if 'complaint_comments' not in existing_tables:
        op.create_table(
            'complaint_comments',
            sa.Column('id', sa.UUID(), nullable=False),
    ...
    # 4. Create notice_read_receipts table if not exists
    if 'notice_read_receipts' not in existing_tables:
        op.create_table(
...
def downgrade() -> None:
    """Downgrade schema safely using inspector."""
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    existing_tables = set(inspector.get_table_names())

    if 'notice_read_receipts' in existing_tables:
        op.drop_table('notice_read_receipts')
    if 'notices' in existing_tables:
        op.drop_table('notices')
    if 'complaint_comments' in existing_tables:
        op.drop_table('complaint_comments')
    if 'complaint_tickets' in existing_tables:
        existing_indexes = {ix['name'] for ix in inspector.get_indexes('complaint_tickets')}
        if 'ix_complaint_tickets_ticket_number' in existing_indexes:
